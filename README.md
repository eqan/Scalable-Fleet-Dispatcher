...
- **`ci.yml`** runs on pull requests and pushes to `main`:
  ...
  - runs Prettier format check (`bun run format:check`) — CI stays check-only so the gate is real
  - locally: `make format` or `make install-hooks` (pre-commit auto-formats before commit)
